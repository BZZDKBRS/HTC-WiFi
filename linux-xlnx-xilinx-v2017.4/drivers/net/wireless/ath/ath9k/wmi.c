...
//rt-wifi的定时中断
		...
		case WMI_GENTIMER_EVENTID:
			ath9k_hw_disable_interrupts(ah);	//原来的代码也没有ah
			ath_rt_wifi_tasklet(priv);         //引向rt-wifi 发送函数
			ath9k_hw_enable_interrupts(ah);
		break; //这个break要不要加目前还有疑问
#endif
			      ...
			      enum htc_endpoint_id epid)
{
	...
	if (cmd_id & 0x1000) {
		...
		tasklet_schedule(&wmi->wmi_event_tasklet);//触发tasklet
		return;
	}
