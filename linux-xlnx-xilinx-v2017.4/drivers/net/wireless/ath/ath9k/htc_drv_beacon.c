				  ...
				  bool reset_tsf)
{
	 ...
	 * set the beacon response time to a larger value.
	 为了减少在重 TX 负载下的信标未命中，请将信标响应时间设置为更大的值。
	 */
				       ...
				       struct ath_beacon_config *conf)
{
	...
	ath9k_htc_beacon_init(priv, conf, false);
				...
				if(priv->rt_wifi_timer == NULL) {
					RT_WIFI_DEBUG("No timer is allocated.\n");
				} else {
					RT_WIFI_DEBUG("AP timer starts.\n");
					ath_rt_wifi_ap_start_timer(priv, conf->intval, conf->nexttbtt);	//rt: (sc, intval, nexttbtt)
					priv->rt_wifi_enable = 1;
				}
#endif
				  ...
				  int slot)
{
	...
	/* Get a new beacon */
	beacon = ieee80211_beacon_get(priv->hw, vif);	//rt :beacon -> skb
	if (!beacon) {
		spin_unlock_bh(&priv->beacon_lock);
		return;
	}
#ifdef CPTCFG_RT_WIFI
	/* Append TDMA information to a beacon frame by vendor specific info. 通过供应商特定信息将 TDMA 信息附加到信标帧*/
	if (priv->rt_wifi_enable == 1) {
		//priv->rt_wifi_bc_tsf初值为0//
		priv->rt_wifi_bc_tsf += RT_WIFI_BEACON_INTVAL;
		//priv->rt_wifi_bc_asn初值为0//
		priv->rt_wifi_bc_asn +=
			RT_WIFI_BEACON_INTVAL / priv->rt_wifi_slot_len;

		tmp = skb_put(beacon, RT_WIFI_BEACON_VEN_EXT_SIZE);  //rt :beacon --> skb
		tmp[0] = RT_WIFI_BEACON_TAG;	/* Tag number for Vendor Specific Info */
		tmp[1] = 0x0F;	  /* Lengh of tag (exclude these two bytes) */

		src = (unsigned char *)(&priv->rt_wifi_bc_asn);
		memcpy((tmp+2), src, sizeof(int));

		src = (unsigned char *)(&priv->rt_wifi_bc_tsf);
		memcpy((tmp+6), src, sizeof(u64));

		*(tmp+14) = RT_WIFI_TIME_SLOT_LEN;

		src = (unsigned char *)(&priv->rt_wifi_superframe_size);
	 ...
	 * add this value for every beacon.
	 在此处更新 TSF 调整值，HW 将为每个信标添加此值。
	 */

	//POI： 这个就是AP将自己的tsf写入到beacon里面，作为时间戳//
	/*yang:用ieee80211_mgmt结构体将skb->data中的前面若干个字段提取出来
	（包括frame_control,duration,da,sa,bssid,seqctrl,
	以及beacon帧特有的timestamp,beacon interval,capability information,variable字段），
	并对其timestamp字段进行数据的写入*/
	...
	if (info->flags & IEEE80211_TX_CTL_ASSIGN_SEQ) {
		/*
		 * TODO: make sure the seq# gets assigned properly (vs. other TX frames)
		 确保 seq# 被正确分配（与其他 TX 帧相比）
		 */
		struct ieee80211_hdr *hdr =
	...
	tx_fhdr = skb_push(beacon, sizeof(beacon_hdr));   //向skb数据区头部添加数据
	...
	ath9k_htc_csa_is_finished(priv);//rt---sub
}
...
//rt---add
	if (priv->ah->opmode != NL80211_IFTYPE_AP &&
		...
		ath_dbg(common, BEACON, "slot 0, tsf: %llu\n",
			be64_to_cpu(swba->tsf));
		return 0;
	}
//end

	//intval = priv->cur_beacon_conf.beacon_interval;
	intval = priv->cur_beacon_conf.beacon_interval ? : ATH_DEFAULT_BINTVAL;  //rt---modify
	tsf = be64_to_cpu(swba->tsf);
//	tsf += TU_TO_USEC(priv->ah->config.sw_beacon_response_time);        //rt---add
	tsftu = TSF_TO_TU(tsf >> 32, tsf);//推出结论是保留第22位-54位
		    ...
		    struct wmi_event_swba *swba)
{
	...
	if (swba->beacon_pending != 0) {
		//这里计算错过多少个beacon帧//
		priv->beacon.bmisscnt++;				//rt:priv->cur_beacon_conf.bmiss_cnt++;
		if (priv->beacon.bmisscnt > BSTUCK_THRESHOLD) {				//rt:priv->cur_beacon_conf.bmiss_cnt
	...
	if (priv->beacon.bmisscnt) {			//rt:priv->cur_beacon_conf.bmiss_cnt
		ath_dbg(common, BSTUCK,
			"Resuming beacon xmit after %u misses\n",
			priv->beacon.bmisscnt);			
		priv->beacon.bmisscnt = 0;			//rt:priv->cur_beacon_conf.bmiss_cnt
	}
...
//POI : 他这里将beacon的载波监听和退避机制关了！//
#ifdef CPTCFG_RT_WIFI
			REG_SET_BIT(priv->ah, AR_DIAG_SW, AR_DIAG_FORCE_CH_IDLE_HIGH);//这里把ah换成了priv->ah
			REG_SET_BIT(priv->ah, AR_DIAG_SW, AR_DIAG_IGNORE_VIRT_CS);
			REG_SET_BIT(priv->ah, AR_D_GBL_IFS_MISC, AR_D_GBL_IFS_MISC_IGNORE_BACKOFF); 
#endif
